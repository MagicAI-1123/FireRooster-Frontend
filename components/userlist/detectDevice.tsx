...
export const detectDevice = (navigator: NavigatorExtended): string => {
  let check = false;
  (function (a: string) {
    if (
...
export const getLocation = async (): Promise<LocationResponse | null> => {
  const API_KEY = process.env.NEXT_PUBLIC_APP_IP_DETECTION_API_KEY;
  if (!API_KEY) {
    console.error('IP detection API key is not configured');
    return null;
  }
...
export const detectBrowser = (navigator: NavigatorExtended): string => {
...
export const detectOS = (navigator: NavigatorExtended): string | null => {
  const userAgent = navigator.userAgent;
